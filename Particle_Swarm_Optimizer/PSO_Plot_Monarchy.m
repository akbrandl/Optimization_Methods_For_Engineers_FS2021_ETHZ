function [glob_best_F,glob_best_pos,store_F,store_position,iter,fnc_eval] = ...
    PSO_Plot_Monarchy(fnc,dim,max_iter,N,lb,ub,w_in,a1_in,a2_in,vel_rel,CODE_SELECT)
%% Paramteter Initialization %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Initialization of variables for plotting (ATTENTION: Memory Consuming!)
store_F = []; store_position = []; 
...
max_step_size = (ub-lb).*vel_rel; 
   ub_velocity = max_step_size;        
   lb_velocity = -ub_velocity;                     
   
%% Monarch to Worker Ration
    ratio = 0.333;    %% Let at firt 30% of all particles be selfish monarchs
    N_particles = 1:1:N; 
...
for i = 1:N     %% Generation of N particles & storage in vector 'swarm'
    ...
    fnc_eval = fnc_eval + 1;  
     ...
     store_position(1,:,i)=newborn.pos; 
end
...
if isequal(CODE_SELECT(1),'l') && ~isequal(CODE_SELECT(2),'f') %% Local PSO
    while StoppingCriteria(iter, max_iter,glob_best_F) 
        ...
        for i = 1:N
            ...
            store_position(iter+1,:,i)=swarm(i).pos;
        end
        
        % Store Current global best...
        store_F = [store_F,glob_best_F];
    end
